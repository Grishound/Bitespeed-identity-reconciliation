...
def identify_contact():
    ...
    if not email and not phone_number:
        ...
        existing_primary_contacts = get_existing_primary_contacts(
        ...
        if not existing_primary_contacts:
            ...
            secondary_contacts = Contact.query.filter_by(
        ...
        if not existing_primary_contacts:
            ...
            if len(existing_primary_contacts) == 2:
                ...
                extra_secondary_contacts = Contact.query.filter_by(
                    linkedId=existing_primary_contacts[1].id
                ).all()
                ...
                email_exists = db.session.query(
                    db.exists().where(Contact.email == email)
                ).scalar()
                phone_number_exists = db.session.query(
                    db.exists().where(Contact.phoneNumber == phone_number)
                ).scalar()
